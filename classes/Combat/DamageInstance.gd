...
func apply_modifiers(skill:Skill, attacker:Pawn2D, target:Pawn2D) -> void:
...
func apply_resistances(pawn:Pawn2D) -> void:
	var cs:CombatStats = pawn.get_cs();
	multiplier *= cs.resistances.get(_Enums.get_key("DMG_ELEMENTS", element), 1.0);
	multiplier *= cs.resistances.get(_Enums.get_key("DMG_TYPES", type), 1.0);

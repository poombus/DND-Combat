...
@onready var type = $ScrollContainer/VBoxContainer/Type/OptionButton;
...
func _ready():
	...
	for e in _Enums.NPC_TYPES.keys(): 
		type.add_item(e);
		type.set_item_metadata(type.item_count-1, _Enums.NPC_TYPES[e]);
...
func _on_button_pressed():
	...
	charsheet.type = type.get_item_metadata(type.get_selected_id());
